...
class TRPO:
    def __init__(self, hidden_dim, state_space, action_space, lmbda, kl_constraint, alpha, critic_lr, gamma, device):
        ...
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=critic_lr)  # 算法给出了策略网络的优化方法，所以不用优化器，只有价值网络有优化器
        self.gamma = gamma
    ...
    # hessian_matrix_vector_product 用于计算共轭梯度算法中的海森矩阵与向量乘积的结果，该方法避免了直接计算和存储海森矩阵
    def hessian_matrix_vector_product(self, states, old_action_dists, vector):
        """
        state: [T, state_dim], T 表示轨迹长度
        old_action_dists: [T, action_dim]
        vector: [T, parameters_nums]
        """
    ...
    def compute_surrogate_obj(self, states, actions, advantage, old_log_probs, actor):
        ...
        states: [T, state_dim]
        actions: [T, 1]
        advantage: output of compute_advantage
        old_log_probs: logarithm of the actor's output.
        actor: PolicyNet
        """
        log_probs = torch.log(actor(states).gather(1, actions))  # 先对数再指数的方式数值稳定性好，避免两个极小数相除
        ratio = torch.exp(log_probs - old_log_probs)
        return torch.mean(ratio * advantage)

    def line_search(self, states, actions, advantage, old_log_probs, old_action_dists, max_vec):
        """ max_vec: sqrt(2 * kl_constraint / x^T * H * x) * x """
