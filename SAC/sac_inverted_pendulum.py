...
class PolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x: [T, state_dim], state
        return:
            action: [T, action_dim], action
            log_prob: [T, 1] 概率密度函数值的对数
        """
        x = F.relu(self.fc1(x))
        mu = self.fc_mu(x)
        ...
        # 1-torch.tanh(action).pow(2)减去这个东西是根据变量变换公式推出来的
        log_prob = log_prob - torch.log(1 - torch.tanh(action).pow(2) + 1e-7)  # 1-torch.tanh(action).pow(2)是双曲正切函数的导数，
...
agent = SACContinuous(state_dim, hidden_dim, action_dim, action_bound, actor_lr, critic_lr, alpha_lr, target_entropy, tau,
                      gamma, device)
