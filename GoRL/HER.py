...
class WorldEnv:
    ...
    def reset(self):
        self.goal = np.array([4 + random.uniform(-0.5, 0.5), 4 + random.uniform(-0.5, 0.5)])  # 生成一个目标状态, 坐标范围是[3.5~4.5, 3.5~4.5]
        self.state = np.array([0, 0])  # 初始状态
        self.count = 0
        return np.hstack((self.state, self.goal))  # 状态和目标拼接，前两个数是当前状态，后两个数是目标

    def step(self, action):
        action = np.clip(action, -self.action_bound, self.action_bound)
        x = max(0, min(5, self.state[0] + action[0]))  # 根据当前状态和输入动作计算下一个状态，网格范围0~5
        y = max(0, min(5, self.state[1] + action[1]))
        self.state = np.array([x, y])
        self.count += 1

        dis = np.sqrt(np.sum(np.square(self.state - self.goal)))  # 当前状态与目标的距离，欧氏距离
        reward = -1.0 if dis > self.distance_threshold else 0     # 距离比目标距离大奖励是-1，小于等于就是0
...
"""
目标导向强化学习相比于传统强化学习的差别是目标导向强化学习可以允许不同的目标，策略网络和价值网络的输入也多了目标。
同一个状态s，在不同目标g下，最优动作是不同的。需要换目标时直接输入新g即可。
"""
...
class DDPG:
    def __init__(self, state_dim, hidden_dim, action_dim, action_bound, actor_lr, critic_lr, sigma, tau, gamma, device):
        ...
        self.target_critic.load_state_dict(self.critic.state_dict())  # 初始化目标价值网络并使其参数和价值网络一样
        self.target_actor.load_state_dict(self.actor.state_dict())    # 初始化目标策略网络并使其参数和策略网络一样
...
class ReplayBufferTrajectory:
    ...
    def sample(self, batch_size, use_her, dis_threshold=0.15, her_ratio=0.8):
        ...
        for _ in range(batch_size):
            ...
            state = traj.states[step_state]              # state shape: (4, )
            next_state = traj.states[step_state + 1]     # next_state shape: (4, )
            action = traj.actions[step_state]            # action shape: (2, )
            reward = traj.rewards[step_state]            # float
            done = traj.dones[step_state]                # bool

            if use_her and np.random.uniform() <= her_ratio:
                step_goal = np.random.randint(step_state + 1, traj.length + 1)  # np.random.randint取不到后面的数
                goal = traj.states[step_goal][:2]                               # 使用HER算法的future方案设置目标
                dis = np.sqrt(np.sum(np.square(next_state[:2] - goal)))         # 在当前状态采取动作后与目标的距离
...
#             if replay_buffer.size() >= minimal_episodes:
#                 for _ in range(n_train):
#                     transition_dict = replay_buffer.sample(batch_size, False)  # 和使用HER训练的唯一区别
#                     agent.update(transition_dict)
#             if (i_episode + 1) % 10 == 0:
#                 pbar.set_postfix({'episode': num_episodes / 10 * i + i_episode + 1, 'return': np.mean(return_list[-10:])})
#             pbar.update(1)
...
# plt.xlabel('Episodes')
# plt.ylabel('Returns')
# plt.title('DDPG without HER on GridWorld')
# plt.show()
