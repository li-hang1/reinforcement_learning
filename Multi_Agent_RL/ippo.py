...
class PPO:
    ...
    def update(self, transition_dict):
        states = torch.from_numpy(np.stack(transition_dict['states'])).float().to(self.device)
        actions = torch.from_numpy(np.stack(transition_dict['actions'])).view(-1, 1).to(self.device)
        rewards = torch.tensor(transition_dict['rewards'], dtype=torch.float).view(-1, 1).to(self.device)
        next_states = torch.from_numpy(np.stack(transition_dict['next_states'])).float().to(self.device)
    ...
    with tqdm(total=int(num_episodes / 10), desc=f'Iteration {i}') as pbar:
        for i_episode in range(int(num_episodes / 10)):
            ...
            if (i_episode + 1) % 100 == 0:
                pbar.set_postfix({'episode': num_episodes / 10 * i + i_episode + 1, 'return': np.mean(win_list[-100:])})
            pbar.update(1)
